...
fil = "res_new/simP.Standard.RDS"
if(file.exists(fil))
  StandardINLA = readRDS(fil)
if(!file.exists(fil))
{
  StandardINLA = inla_KnorrHeld4(df,Q_RW2,Q_ICAR,family="poisson",scale=sc)
  saveRDS(StandardINLA,file=fil)
}

fil = "res_new/simP.HyMiK.RDS" 
if(file.exists(fil))
  HyMiK = readRDS(fil)
if(!file.exists(fil))
{
  HyMiK = inla_KnorrHeld4(df,Q_RW2,Q_ICAR,family="poisson",method="hymik",scale=sc)
  saveRDS(HyMiK,file=fil)
}

fil = "res_new/simP.HyBW.RDS"
if(file.exists(fil))
  HyBW = readRDS(fil)
if(!file.exists(fil))
{
  ...
  HyBW = inla_KnorrHeld4(df,Q_RW2,Q_ICAR,family="poisson",method="hybw",scale=sc)
...
if(!file.exists(fil))
{
  HyPrick = inla_KnorrHeld4(df,Q_RW2,Q_ICAR,family="poisson",method="hyprick",scale=sc)
...
print(tab)
print(xtable(tab,digits=3),type="latex",file="tab_simP.tex")
...
ggsave("res_new/SimP_Interaction_E_sd.png",device="png",height=5,width=5)
}