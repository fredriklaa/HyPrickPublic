...
I=INLA::inla.graph2matrix(graph)[1:ns,1:ns]
diag(I)=0
I=-I
diag(I)=-rowSums(I)
...
Q_RW2=inla.scale.model(GMRF_RW(order=2,n=10),constr = list(A=rbind(rep(1,nt),seq(1,nt)),e=rep(0,2)))
Q_ICAR=inla.scale.model(I2+Diagonal(ns)*epsilon,constr=list(A=matrix(rep(1,ns),nrow=1),e=0))
Q_st=kronecker(Q_RW2,Q_ICAR)

#Simulate from field
x_t_main=inla.qsample(n=1,Q=Q_RW2+Diagonal(nt)*epsilon,seed = 1)
x_s_main=inla.qsample(n=1,Q=Q_ICAR+Diagonal(ns)*epsilon,seed = 1)
x_st_main=inla.qsample(n=1,Q=Q_st+Diagonal(ns*nt)*epsilon,seed = 1)

#Correct for constraints: Conditioning by kriging: 
x_t_main=inla.qsample(n=1,Q=Q_RW2+Diagonal(nt)*epsilon,mu = x_t_main,constr = list(A=A_t,e=c(0,0)),seed = 1)
x_s_main_main=inla.qsample(n=1,Q=Q_ICAR+Diagonal(ns)*epsilon,mu = x_s_main,constr = list(A=A_s,e=c(0)),seed = 1)
x_st_main=inla.qsample(n=1,Q=Q_st+Diagonal(ns*nt)*epsilon,mu = x_st_main,constr = list(A=A_st,e=rep(0,nrow(A_st))),seed = 1)
...
mean_pred=0.1*DesignMatrixMainTemporal%*%as.vector(x_t_main)+0.1*DesignMatrixMainSpatial%*%as.vector(x_s_main)+0.1*DesignMatrixMainInteraction%*%as.vector(x_st_main)
...
data_in$Y=rpois(n=length(mean_pred),lambda=exp(2.5+mean_pred))
#class(data_in$Y) = "integer"
show(range(data_in$Y))

SpatioTemporalDataSmall = data_in
saveRDS(SpatioTemporalDataSmall,file=paste0(dirdata,"SpatioTemporalDataSmall.RDS"))
saveRDS(list(Q_RW2=Q_RW2,Q_ICAR=Q_ICAR),file=paste0(dirdata,"SpatioTemporalDataSmall_Q.RDS"))
