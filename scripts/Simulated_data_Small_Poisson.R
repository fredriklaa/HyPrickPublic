...
if(0)
{
tab =cbind(StandardINLA$summary.hyperpar$mean,BW$summary.hyperpar$mean,HyMiK$summary.hyperpar$mean,
           HyBW$summary.hyperpar$mean,HyPrick$summary.hyperpar$mean)
tab = rbind(c(StandardINLA$summary.fixed$mean,BW$summary.fixed$mean,HyMiK$summary.fixed$mean,
              HyBW$summary.fixed$mean,HyPrick$summary.fixed$mean),
            tab,c(StandardINLA$cpu.used[4],BW$cpu.used[4],HyMiK$cpu.used[4],HyBW$cpu.used[4],HyPrick$cpu.used[4]))
colnames(tab) = c("Standard","BW","HyMiK","HyBW","HyPrick")
...
p1 = ggplot(data = plotDataE) +
  ...
  scale_colour_manual(
    name = "Method",
    values = c("BW" = "pink", "HyPrick" = "black", "HyMiK" = "red", "HyBW" = "blue")
  ) +
